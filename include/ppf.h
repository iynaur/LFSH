...
#define NDist  16
#define NAngle  32
#define PPFHistDim NDist * NAngle

typedef pcl::Histogram<PPFHistDim> PPFHistogram;
POINT_CLOUD_REGISTER_POINT_STRUCT(PPFHistogram, (float[PPFHistDim], histogram, histogram))

namespace pcl
{
template<typename PointT,typename PointNT,typename PointOUTT>
class PPFEstimation : public FeatureFromNormals<PointT,PointNT,PointOUTT>
{
public:
    typedef boost::shared_ptr<PPFEstimation<PointT,PointNT,PointOUTT> > Ptr;
    typedef boost::shared_ptr<const PPFEstimation<PointT,PointNT,PointOUTT> > ConstPtr;
